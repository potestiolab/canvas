...
from inp_out import *   
#from write_lammps_files import *     ### NOT MPI 
from write_lammps_files_MPI import * 
from read_gromacs_input_files_MPI import * 
...
#F1. Finding survived atoms, and not-survived atoms
def surv_atoms(dict_survived, i):
    oth = None 
    surv = None 
    flag_surv = False 
...
#F2. Compute "min_distances" list of lists that contains 
def min_distance(list_survived_atoms, i):
    index = 0
    dist = []
    min_dist=1000000
...
#F3 For each survived atom and other ones (decimated atoms) create a inner list containing the value of reference block and relative Charge 
#   [(block, Q), (block, Q)...] 
def update_1(block_list, i):
...
#F4- For each survived atom, we substitute the value of corresponding total charge:
#    o For CG beads it is such sum 
#    o For AT atoms the charge is unvariate and it remains the original one. 
def update_2(Q_list, k):
    for j in Q_list:                                                   # Change the value of charge k[-1] for each survived atom
        if(j[0]==k[9] and (k[7]=="cg" or k[7]=="CG" or k[7]=="Cg")):   # with the value if the charge total sum of each block 
            k[-1] = float("{:3.6f}".format(j[1]))

    return k


# F5- For each survived atom and other ones (decimated atoms) create a inner list containing the value of reference block and relative x,y,z values 
#     [(block, x,y,z), (block, x,y,z)...] 
def update_sigma1(block_list,i):
    for block in block_list:
        if(block==i[9]):
            return block,i[4],i[5],i[6]               #i[4] = x  i[5]=y  i[6]=z         --> return a tuple (block_ref, x,y,z) 


# F6- For each atom, (x-xCM)^2 ; (y-yCM)^2 ; (z-zCM)^2 is computed. 
def update_sigma2(CM_list, j):
    for i in CM_list:
        if(i[0]==j[0]):
...
# F7- Updating the list of survived atoms with the value of sigma, equals to 2*Rg in case if CG (or MG) bead. Otherwise, the default
#     value of sigma of the specific atomtype is appended. 
    
def update_sigma3(Rg_list, dict_sigma, i):
    for block, Rg, n_atoms_block in Rg_list:

        if(n_atoms_block!=1 and block==i[9]):              # n_atoms_block =! 1 and block==i[9]
            i.append(Rg)                                   # append Rg 
...
# F8- Calculating the original value of epsilon for each atom comparing with dict_epsilon, and adding it in "list_survived_atoms"  
#     and "list_other_atoms". 

def update_eps(dict_epsilon, i):
    for k,v in dict_epsilon.items():
        if(k==i[8]):
            return i + [v]


# F9- For each survived atom and other ones (decimated atoms) create a inner list containing the value of reference block and relative Log(Eps) 
#     [(block, Log(Eps)), (block, log(Eps))...] 
def update_eps1(block_list,i):
    for block in block_list:
...
# F10- The list of survived atoms is updated with the value of Epsilon (in case of AT survived atoms, the original value of epsilon is token in account)
def update_eps2(eps_list, i):
    for block, eps, count_eps in eps_list:
        if(block==i[9]):
            i[-1] = eps 
    return i
...
#F11- Updating list_survived_atoms (i) with label "One", "Four" or "at", if the atom is coarse-grained, medium-grained or all-atom, respectively    
def update_Four_One_Atom(dict_coun, i):
    for k,v in dict_coun.items():
        if(k==i[0]):
            return i + [v]


#F12- Updating "atom" with the atomic number (H=1, C=6, and so on...) for each survived atom. It can be found it in dict_atnum. 
def update_atomic_number(dict_atnum, i):
    for k,v in dict_atnum.items():
        if(i[8]==k):
            return i + [v]


#F13- Checking if two survived atoms have a covalent bond in the fully-atomistic representation 
def update_A1(list_survived_only_num, i):
    result=all(x in list_survived_only_num for x in i)
    if(result==True):
        return i


#F14- Writing the at_type and at/cg for each atom involved in the bonds
def update_A2(bond_prot_fully_at, k):
    A = []
    for i in bond_prot_fully_at:
        if(k[3] == i[0] or k[3]==i[1]):
            A = (k[3], k[8],k[7])

    return A


#F15- For each couple of atoms involved in the bonds, the list 'bond_prot_fully_at' will be updated with the 'at_type' and the 'at/cg', and 
#     afterwards, a swop between i[3] and i[4] elements is done, in order to have the followiing list of lists: 
#       
#     [[a1, a2, at_type-1, at_type-2, at/cg-1, at/cg-2], [...]]

def update_A2b(upd_A2, i):
    for j in upd_A2:
        if(i[0] == j[0]):
            i.append(j[1])
            i.append(j[2])
...
#F16- After reading the bondtype in ffbonded file (ffbon[0]), we append the value if b0 and kb for each couple of 
...
#F17- Reconverting the original index (of fullyAT representation) in the new index after decimated the most of atoms in CANVAS model
def update_A4(bond_prot_fully_at, i):
    A = []
    for j in bond_prot_fully_at:
        if(i[3] == j[0] or i[3] == j[1]):
            A = (i[3], i[9])

    return A

#F18- Substituting the original value (1st and 2nd column) after reconversion
def update_A4b(upd_A4, i):
    for j in upd_A4:
        if(i[0] == j[0]):
            i[0] = j[1]
...
#F19- Definining variable bonds according with di distance between beads. 
def add_variable_bonds_ENM(i, j, orig):
...
#F20 - Defining strong and weak bonds
def add_bonds_ENM(k_el, i, j, orig):
...
#F19. Updating bonds according with atoms belong to consecutive or not residues, their distance, and resolution (CG, MG, or AT) 
def update_bonds(list_survived_atoms, i):
    k_interface = 50    # elastic constant on the interface AT-CG where the beads and atoms involved are not consecutive
    k_strong  = 50000   # elastic constant for beads that belong to consecutive residues.  
    orig = []
    for j in list_survived_atoms:
        ...
        if(j[9]>i[9]):                                                              # We avoid repeated elements (at_number of J > at_number of I)
            ...
            if(i[7] =="cg" and i[14]=="one" and j[14]=="atom"):                     # i = cg-CA, j = at-residue 
                ...
                if(i[0] == j[0] - 1):                                               # i (cg-CA) precedes j (at-residue)
                    ...
                    if(j[2]=="N"):                                                  # i = cg-CA, j = atomistic N 
                        add_bonds_ENM(k_strong, i, j, orig)          
                ...
                if(i[0] != j[0] - 1 and i[0] != j[0]):  
                    ...
                    if(j[2] =="CA"):				                    # i = cg-CA, j = atomistic CA
                        add_bonds_ENM(k_interface, i, j, orig) 
            ...
            if(i[7] =="cg" and i[14]=="one" and j[7] == "cg" and j[14]=="one"):     # i = cg-CA ; j = cg-CA 
                ...
                # If NOT-consecutive
                if(i[0] != j[0] - 1 and i[0] != j[0]):
                    add_variable_bonds_ENM(i,j, orig)
...
#F20- Updating about domains     
def update_doms(list_domains, dom_fullyAT, i):
    for count in list_domains:
        if(i[3] in dom_fullyAT[count] and i[7] =="cg"):
            return count, i[9]


#F21- Checking if two survived atoms have a pair in the fully-atomistic representation  
def update_B1(list_survived_only_num, i):
    result=all(x in list_survived_only_num for x in i)
    if(result==True):
        return i


# F22- Calculating, for each atom involved in pairs, the at_type, at/cg and the NewIndex after reconversion of index number. 
def update_B2(pairs_mult_res_prot, k):
    A = []
    for i in pairs_mult_res_prot:
        if(k[3]==i[0] or k[3]==i[1]):
            A = (k[3], k[8],k[7])
            A = (k[3], k[8],k[7],k[9])         #k[8] = type; k[7]=at/cg; k[9]=NewIndex

    return A


# F23- Appending, in 'pairs_mult_res_prot' list, the at_type, at/cg value and substituting the NewIndex value after reconversion.
#      Finally, a swap of elements i[3] and i[4] is done.  
def update_B2b(upd_B2, i):
    for j in upd_B2:
        if(i[0] == j[0]):
            i.append(j[1])  # append type  
            i.append(j[2])  # append at/cg 
            i[0] = j[3]     # substitute NewIndex
...
# F24- After reading the pairtype in ffnonbonded file (ffnonbon[4]), we append the value if "sigma14" and "epsilon14" for each couple of 
...
def update_s14_eps14(pairtypes, i):
    for j in pairtypes:
        if((j[0]==i[2] and j[1]==i[3]) or (j[0]==i[3] and j[1]==i[2])):
            i.append(float(j[2]))
            i.append(float(j[3]))
...
#F25- Checking if three survived atoms have a angle in the fully-atomistic representation 
def update_C1(list_survived_only_num, i):
    result=all(x in list_survived_only_num for x in [i[0],i[1],i[2]])
    if(result==True):
        return i


#F26- Creating, for each atom involved in the angle potential, the at_type, at/cg, NewIndex and the NewType             
def update_C2(angles_prot_fully_at, k):
    A = []  
    for i in angles_prot_fully_at:
        if(k[3]==i[0] or k[3]==i[1] or k[3]==i[2]):
            A = (k[3], k[8],k[7],k[9],k[16])         #k[8] = type; k[7]=at/cg; k[9]=NewIndex, k[16]=A{i} in case of CGbead, otherwise i[16]=type
            
    return A


#F27- Appending, in 'angles_prot_fully_at' the type, at/cg, NewType and substituting the NewIndex 
def update_C2b(upd_C2, i):
    for j in upd_C2:
        if(i[0] == j[0]):
            i.append(j[1])   # append type (old atomtype in case of AT, a new atomtype i.e. A{i} in case of CG-bead) 
            i.append(j[2])   # append at/cg 
            i.append(j[4])   # append NewType  
            i[0] = j[3]      # Substitute NewIndex
            break
...
#F28- After reading the angletypes in ffbonded file (angletypes = ffbon[1]), we append the value of angle elastic constant (cth) 
#     and the angle (th0) for each triplet of survived atoms
#
#     Basically angletypes and angles_prot_fully_at look like as follow: 
#
#               angles_prot_fully_at : [[1,2,3, func, X,at/cg,NewType,  Y,at/cg,NewType,  Z,at/cg,NewType],[2,3,5, A,..,B...,C...],[...]]
#               angletypes           : [[X,Y,Z, func, th0,cth],[...]]                      if func = 1 
#               angletypes           : [[X,Y,Z, func, th0,cth,ub0,kub],[...]]             if func = 5
# 
#                                                              where 'func' could be 1 or 5 (usually). 

def update_cth_th0(angletypes, i):
    for j in angletypes:
        if((j[0]==i[4] and j[1]==i[7] and j[2]==i[10]) or (j[0]==i[10] and j[1]==i[7] and j[2]==i[4])):
            i.append(float(j[5]*8.368))
            i.append(float(j[4]))             # Append cth value (in Groamacs units) and th0 (Gromacs units) 
...
#F29- Checking if four survived atoms have a dihedral/torsion in the fully-atomistic representation
def update_D2(list_survived_only_num, i):
    result=all(x in list_survived_only_num for x in [i[0],i[1],i[2],i[3]])
    if(result==True):
        return i

#F30- Creating a tuple cointaining, for each atom (or bead) involved in dihedrals, the type, at/cg, NewIndex, NewType
def update_D3(dihedrals_prot_fully_at, k):
    A = []
    for i in dihedrals_prot_fully_at:
        if(k[3]==i[0] or k[3]==i[1] or k[3]==i[2] or k[3]==i[3]):
            A = (k[3], k[8],k[7],k[9],k[16])         #k[8] = type; k[7]=at/cg; k[9]=NewIndex, k[16]=A{i} in case of CGbead, otherwise i[16]=type

    return A


#F31- Append in dihedrals_prot_fully_at_(dih2, dih4, dih9, tors) the type, at/cg, the NewType and substituting the NewIndex. 
def update_D3b(upd_D3, i):
    for j in upd_D3:
        if(i[0] == j[0]):
            i.append(j[1])   # append type (old atomtype in case of AT, a new atomtype i.e. A{i} in case of CG-bead) 
            i.append(j[2])   # append at/cg 
...
#F32- After reading the dihedraltypes in ffbonded file,
...
def update_D4(dihedraltypes_dih49, i):
    flag_dih = True
    for j in dihedraltypes_dih49:

        if((j[0]==i[5] and j[1]==i[8] and j[2]==i[11] and j[3]==i[14]) or (j[0]==i[14] and j[1]==i[11] and j[2]==i[8] and j[3]==i[5])):  #consider symmetric 
        ...
        if(flag_dih == True):       # It means that no quartet of atomtypes was found in dihedraltype 4 and give a look to the case in which X is present

            # 4 cases (plus symmetric ones): only one X... 
            if((j[0]=="X" and j[1]==i[8] and j[2]==i[11] and j[3]==i[14]) or (j[0]==i[14] and j[1]==i[11] and j[2]==i[8] and j[3]=="X")): # (1)
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]==i[5] and j[1]=="X" and j[2]==i[11] and j[3]==i[14]) or (j[0]==i[14] and j[1]==i[11] and j[2]=="X" and j[3]==i[5])): # (2)   
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]==i[5] and j[1]==i[8] and j[2]=="X" and j[3]==i[14]) or (j[0]==i[14] and j[1]=="X" and j[2]==i[8] and j[3]==i[5])): # (3)
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]==i[5] and j[1]==i[8] and j[2]==i[11] and j[3]=="X") or (j[0]=="X" and j[1]==i[11] and j[2]==i[8] and j[3]==i[5])): # (4)           
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            # 6 cases (plus symmetric ones): twice X.
            if((j[0]=="X" and j[1]=="X" and j[2]==i[11] and j[3]==i[14]) or (j[0]==i[14] and j[1]==i[11] and j[2]=="X" and j[3]=="X")):   # (1')
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]=="X" and j[1]==i[8] and j[2]=="X" and j[3]==i[14]) or (j[0]==i[14] and j[1]=="X" and j[2]==i[8] and j[3]=="X")):   # (2')
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]=="X" and j[1]==i[8] and j[2]==i[11] and j[3]=="X") or (j[0]=="X" and j[1]==i[11] and j[2]==i[8] and j[3]=="X")):   # (3')
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]==i[5] and j[1]=="X" and j[2]=="X" and j[3]==i[14]) or (j[0]==i[14] and j[1]=="X" and j[2]=="X" and j[3]==i[5])):   # (4')
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]==i[5] and j[1]=="X" and j[2]==i[11] and j[3]=="X") or (j[0]=="X" and j[1]==i[11] and j[2]=="X" and j[3]==i[5])):   # (5')
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])

            if((j[0]==i[5] and j[1]==i[8] and j[2]=="X" and j[3]=="X") or (j[0]=="X" and j[1]=="X" and j[2]==i[8] and j[3]==i[5])):   # (6')
                i.append(j[5])
                i.append(j[6]*4.184)
                i.append(j[7])
...
#F33- After reading the special dihedraltypes defined as torsion (dihedraltypes_tors = ffbon[4]), 
#     we append the value of the dihedral elastic constant (k_phi), the phase (phi0) and the multiplicity (n)
...
def update_D7(dihedraltypes_tors, i):
...
#F34- About dih2, the situation is analogue to dih4 and dih9, but dih2 do not have multiplicity (n) that 
...
def update_D8(dihedraltypes_dih2, i):
    flag_dih2 = True
...
#F35- Checking if five survived atoms have a Torsional Correction Map in fullyAT representation (only for charmm.ff)
def update_E1(list_survived_only_num, i):
    result=all(x in list_survived_only_num for x in [i[0],i[1],i[2],i[3],i[4]])
    if(result==True):
        return i


#F36- Creating a tuple, containing for each atom involved in the cmap correction, the type, at/cg, NewIndex and the NewType 
def update_E2(cmaps_prot_fully_at, k):
    A = []
    for i in cmaps_prot_fully_at:
        if(k[3]==i[0] or k[3]==i[1] or k[3]==i[2] or k[3]==i[3] or k[3]==i[4]):
            A = (k[3], k[8],k[7],k[9],k[16])         #k[8] = type; k[7]=at/cg; k[9]=NewIndex, k[16]=A{i} in case of CGbead, otherwise i[16]=type

    return A


#F37- Appending in 'cmaps_prot_fully_at' list the type, at/cg, NewType and substituting the NewIndex. 
def update_E2b(upd_E2, i):
    for j in upd_E2:
        if(i[0] == j[0]):
            i.append(j[1])   # append type (old atomtype in case of AT, a new atomtype i.e. A{i} in case of CG-bead) 
            i.append(j[2])   # append at/cg 
...
#F38- After reading the cmaptypes in "cmap.itp" file (cmaptypes = readcmap(f_cmap)), we append the func, grid1, grid2, and 
...
#
#               cmaps_prot_fully_at : [[1,2,3,4,5, func, X,at/cg,NewType,  Y,x,x, Z,x,x, K,x,x, H,x,x],[2,3,5,8,9, func, A,B,C,D,E],[...]]
...
def update_E3(cmaptypes, i):
    for j in cmaptypes:
        if((j[0]==i[6] and j[1]==i[9] and j[2]==i[12] and j[3]==i[15] and j[4]==i[18]) or \
           (j[0]==i[18] and j[1]==i[15] and j[2]==i[12] and j[3] ==i[9] and j[4]==i[6])):
...
#F39- Updating "list_survived_atoms" with all the properties for water and ions, i.e.: 
...
def update_L5a_solv(atoms_ions, j):
    for i in atoms_ions:
...
def update_L5a_notsolv(atoms_box, j):
    for i in atoms_box: 
        if(j[9] == i[3]):    # if new_attype == attype of solvated file-
            j[4] = i[4]
            j[5] = i[5]
            j[6] = i[6]
    return j 
...
#F40- Appending [res_number, res_name, at_name, at_number, pos_x, pos_y, pos_z, "at"] for ions and water
def update_L5c_1(atoms_topol_water, i):
...
def update_L5c_2(atoms_topol_ions, i):
...
#F41- Appending mass of water and ions (i[10]) (dict_mass = ffnonbon[2])
def update_L5e(dict_mass, i):
...
#F42- Appending, the number of atomtype (NUM_OF_ATTYPE) for each suvived atom
def update_L6c(dict_at_types, i):
...
#F43- Appending kb and b0 for the OW-HW bond (it can be found in "bondtypes")
def update_L7e(bondtypes, i):
    for j in bondtypes:
        if(len(i) == 6):
            if((j[0]==i[2] and j[1]==i[3]) or (j[0]==i[3] and j[1]==i[2])):
...
#F44- Appending cth (Angle Energy) and th0 (Theta Angle) for the HW-OW-HW (water) angle (it can be found in "angletypes")
def update_L8c(angletypes, i):                                  
    for j in angletypes:
        if(len(i) == 6):
...
if __name__ == '__main__':
    ...
    list_survived_atoms = [i[0] for i in ALL] 
    list_other_atoms    = [i[1] for i in ALL]
    
    list_survived_atoms = [ i for i in list_survived_atoms if i is not None]    # Removing None string when k==i[3] is not found   
    list_other_atoms    = [ i for i in list_other_atoms if i is not None]
    ...
    min_distances = sorted(min_distances, key = itemgetter(2,1,0))
    ...
    ### 13.2.1- For each survived atom and other ones (decimated atoms) create a inner list containing the value of reference block and relative Charge 
    ###         [(block, Q), (block, Q)...] 
    
    block_list = [i for i in range(1, n_survived+1)]
    ...
    Q_list_sur = pool.map(temp, iterable = list_survived_atoms)
    
    pool.close() 
    pool.join() 
    
    Q_list = Q_list_oth + Q_list_sur
    
    ### 13.2.2- As many other atoms are mapped onto the same reference survived atom (same reference block), we calculate 
    ###         the total charge of the atoms having the same reference block:
    ###           [[Q1, block1],[Q2, block2],[Q3, block1],[Q4,block1],[Q5,block2]...] 
    ###         The final rsult will be:
    ###           [[Q1+Q3+Q4,block1], [Q2+Q5, block2]...]
    ...
    Q_new_list = []
    for key in nums:
        Q_new_list.append([key, sum(d[key])])
    
    Q_list = Q_new_list 
    
    
    ### 13.2.3- For each survived atom, we substitute the value of corresponding total charge:
    ###         For CG beads it is such sum 
    ###         For AT atoms the charge is unvariate and it remains the original one. 
    ...
    coo_sur = pool.map(temp, iterable=list_other_atoms) 
    
    coo_tot = coo_oth + coo_sur
    
    pool.close()
    pool.join()
    
    ## 14.2- For each survived atom and corresponding Reference block, create a list containing the  value of Center Of Mass, and the number of 
    ##       total atoms (survived and decimated ones) that make up the block itself.
    ...
    for lst in coo_tot:
        if lst[0] not in d1:
            d1[lst[0]] = []
            nums.append(lst[0])
        d1[lst[0]].append(lst[1])
    
        if lst[0] not in d2:
            d2[lst[0]] = []
        d2[lst[0]].append(lst[2])
    
        if lst[0] not in d3: 
            d3[lst[0]] = []
        d3[lst[0]].append(lst[3])
    
    CM_list = []
    for block in nums:    
        n_atoms_block = len(d1[block])
    
        sum_x = sum(d1[block])
        sum_y = sum(d2[block])
        sum_z = sum(d3[block])
    
        x_CM = sum_x/n_atoms_block 
        y_CM = sum_y/n_atoms_block
        z_CM = sum_z/n_atoms_block
    
        CM_list.append([block, x_CM, y_CM, z_CM, n_atoms_block])
    
    
    ## 14.3- For each atom, (x-xCM)^2 ; (y-yCM)^2 ; (z-zCM)^2 is computed. 
    ...
    ## 14.4- For each reference block the value of Radius of Gyration and the number of atoms of each block are computed.  
    ...
    for block in nums:
        ...
        Rg_squared = sum_x2 + sum_y2 + sum_z2
        Rg_squared = Rg_squared/n_atoms_block
        Rg         = sqrt(Rg_squared)
        Rg         = 2 * Rg        
    
        Rg         = float("{:12.9f}".format(Rg))    # In this way we are sure that there are 9 decimal after the units, not more numbers 
    
        Rg_list.append([block, Rg, n_atoms_block])
    
    
    ## 14.5- Updating the list of survived atoms with the value of sigma, equals to 2*Rg in case if CG (or MG) bead. Otherwise, the default
    ##       value of sigma of the specific atomtype is appended. 
    ...
    temp = partial(update_sigma3, Rg_list, dict_sigma)
    
    upd_s3 = pool.map(temp, list_survived_atoms)
    
    list_survived_atoms = upd_s3
    
    pool.close()
    pool.join()
    
    
    ## 14.6- Calculating the maximum value of sigma, since when creating .mdp files, the coulomb and VdW cutoff should be equals to 2.5 * max_sigma  
    max_sigma = 0 
    ...
    res_surv  = pool.map(temp, iterable=list_survived_atoms)
    
    list_other_atoms    = res_other
    list_survived_atoms = res_surv
    ...
    ## 15.2.1- For each survived atom and other ones (decimated atoms) create a inner list containing the value of reference block and relative Log(Eps) 
    ##         [(block, Log(Eps)), (block, log(Eps))...] 
    ...
    log_eps_oth = pool.map(temp, iterable=list_other_atoms)
    
    log_eps_list = log_eps_oth + log_eps_sur
    
    pool.close()
    pool.join()
    
    log_eps_list = [i for i in log_eps_list if i is not None]
    
    
    ## 15.2.2- For each block, the sum of Logarithm of Epsilon is computed, as well as, the number of atoms that make up the block itself.  
    nums = []
    d = {}
    for lst in log_eps_list:
        if lst[0] not in d:
            d[lst[0]] = []
            nums.append(lst[0])
        d[lst[0]].append(lst[1])
    ...
    for block in nums:
        sum_log_eps   = sum(d[block])
        n_atoms_block = len(d[block])
    
        log_eps_new_list.append([block, sum_log_eps, n_atoms_block])
    
    log_eps_list = log_eps_new_list
    
    
    ### 15.2.3- For each block, the value of Epsilon is calculated. 
    for i in log_eps_list:
    
        sum_log_eps = i[1] 
        count_eps   = i[2]
    
        if(count_eps != 0):    					     ##if count_eps !=0
            i[1] = float(exp((1/count_eps)*sum_log_eps))  ## This is now Eps_block 
        else:
            i[1] = float(0)
    
    
    eps_list = log_eps_list
    
    
    ### 15.2.4- The list of survived atoms is updated with the value of Epsilon (in case of AT survived atoms, the original value of epsilon is token in account)
    ...
    bond_prot_fully_at = [i for i in bond_prot_fully_at if i is not None] 
    ...
    # bond_prot_fully_at = [a1, a2, at_type-1, at/cg-1, at_type-2, at/cg-2] 
    #
    # Quindi facciamo uno swop di k[3] e k[4] in modo da avere: 
    #
    # bond_prot_fully_at = [a1, a2, at_type-1, at_type-2, at/cg-1, at/cg-2] 
    #
    # E cosi facciamo una sola volta il ciclo FOR  
    ...
    upd_A2 = [i for i in upd_A2 if len(i)>1] 
    
    pool.close()
    pool.join() 
    
    ## A.2.2- For each couple of atoms involved in the bonds, the list 'bond_prot_fully_at' will be updated with the 'at_type' and the 'at/cg', and 
    ##        afterwards, a swop between i[3] and i[4] elements is done, in order to have the followiing list of lists: 
    ##       
    ##        [[a1, a2, at_type-1, at_type-2, at/cg-1, at/cg-2], [...]]
    ...
    bond_prot_fully_at = upd_A2b 
    
    pool.close()
    pool.join() 
    
    for i in bond_prot_fully_at:
        i[3],i[4] = i[4],i[3]     # Swap elements 
    ...
    upd_A4 = [i for i in upd_A4 if len(i) > 1]
    ...
    list_survived_atoms2 = list_survived_atoms
    ...
    ALL_bonds = [i for i in ALL_bonds if len(i)>1]  # Avoid empty lists 
    ALL_bonds = list(itertools.chain(*ALL_bonds))   # merge list of lists in a unique list, and afterwards we split in internal lists of 8 elem.
    
    ALL_bonds = [ALL_bonds[i:i + 8] for i in range(0, len(ALL_bonds), 8)]   # Create a list of lists every 8 elements corresponding to a bond
     
    bonds_surv_proteina = ALL_bonds 
    ...
    if domfile is not None:
        ...
        temp  = partial(update_doms, list_domains, dom_fullyAT)
    ...
    pairs_mult_res_prot = [ i for i in pairs_mult_res_prot if i is not None]
    ...
    # B.2- Calculating, for each atom involved in pairs, the at_type, at/cg and the NewIndex after reconversion of index number. 
    ...
    upd_B2 = [i for i in upd_B2 if len(i)>1]
    
    pool.close()
    pool.join()
    
    
    # B.3- Appending, in 'pairs_mult_res_prot' list, the at_type, at/cg value and substituting the NewIndex value after reconversion.
    #      Finally, a swap of elements i[3] and i[4] is done.  
    ...
    #B.4- After reading the pairtype in ffnonbonded file (ffnonbon[4]), we append the value if "sigma14" and "epsilon14" for each couple of 
    #     atoms that require a rescaled LJ and Coul. 'pairtypes' and 'pairs_mult_res_prot' look like as follow: 
    #
    #               pairs_mult_res_prot : [[1,2,X,Y,at,at],[2,3,A,B,at,at],[...]]
    #               pairtypes           : [[X,Y,sigma14,eps14],[...]] 
    ...
    if(Flag_pairtypes == False):   # i.e. the pairtypes list is empty, then the rescaled14 flag is allowed as optional  
        ...
        if(rescaled14 is not None):
            if(rescaled14 == "y" or rescaled14 == "Y"):
                pairs_mult_res_prot = [x for x in pairs_mult_res_prot if x[4]=="at" or x[5]=="at"]   # Why x[2] and x[3]?? NOT correct 
            ...
            pairs_mult_res_prot = [x for x in pairs_mult_res_prot if x[4]=="at" and x[5]=="at"]   # NOT x[2] and x[3]
    
    
    
    if(Flag_pairtypes == True):   # i.e. the pairtypes list is NOT-empty (charmm.ff), then the rescaled14 flag is not permitted and only at-at pairs are allowed
        if(rescaled14 is not None): 
    ...
    angles_prot_fully_at = [ i for i in angles_prot_fully_at if i is not None]
    
    
    
    # C.2- Creating, for each atom involved in the angle potential, the at_type, at/cg, NewIndex and the NewType 
    ...
    upd_C2 = [i for i in upd_C2 if len(i)>1]
    
    pool.close()
    pool.join()
    
    
    # C.3- Appending, in 'angles_prot_fully_at' the type, at/cg, NewType and substituting the NewIndex 
    ...
    angles_prot_fully_at = upd_C2b
    
    pool.close()
    pool.join()
    
    
    #C.4- After reading the angletypes in ffbonded file (angletypes = ffbon[1]), we append the value of angle elastic constant (cth) 
    #     and the angle (th0) for each triplet of survived atoms
    #
    #     Basically angletypes and angles_prot_fully_at look like as follow: 
    #
    ...
    # C.5- Rewrite 'angles_prot_fully_at' 
    
    angles_prot_fully_at = [[i[0],i[1],i[2],i[3], i[6],i[9],i[12], i[13],i[14],i[15],i[16], i[5],i[8],i[11]] if len(i)==17 \
                            else [i[0],i[1],i[2],i[3], i[6],i[9],i[12], i[13],i[14], i[5],i[8],i[11]] for i in angles_prot_fully_at]
    
    
    # C.6- Creating a new list called "angles_mult_res_prot" (in analogy with bonds and pairs)  which is simply angles_prot_fully_at, 
    ...
    dihedrals_prot_fully_at_dih9 = pool.map(temp, iterable=original_dihedrals_protein_fully_at_dih9)
    dihedrals_prot_fully_at_tors = pool.map(temp, iterable=original_dihedrals_protein_fully_at_tors)
    dihedrals_prot_fully_at_dih2 = pool.map(temp, iterable=original_dihedrals_protein_fully_at_dih2)
    
    pool.close()
    pool.join()
    
    dihedrals_prot_fully_at_dih4 = [i for i in dihedrals_prot_fully_at_dih4 if i is not None]
    dihedrals_prot_fully_at_dih9 = [i for i in dihedrals_prot_fully_at_dih9 if i is not None]
    dihedrals_prot_fully_at_tors = [i for i in dihedrals_prot_fully_at_tors if i is not None]
    dihedrals_prot_fully_at_dih2 = [i for i in dihedrals_prot_fully_at_dih2 if i is not None]
    
    
    
    #D.3- Creating a tuple cointaining, for each atom (or bead) involved in dihedrals, the type, at/cg, NewIndex, NewType
    ...
    upd_D3_dih4 = [i for i in upd_D3_dih4 if len(i)>1]
    
    temp_dih9   = partial(update_D3, dihedrals_prot_fully_at_dih9)
    upd_D3_dih9 = pool.map(temp_dih9, iterable=list_survived_atoms)
    upd_D3_dih9 = [i for i in upd_D3_dih9 if len(i)>1]
    
    temp_tors   = partial(update_D3, dihedrals_prot_fully_at_tors)
    upd_D3_tors = pool.map(temp_tors, iterable=list_survived_atoms)
    upd_D3_tors = [i for i in upd_D3_tors if len(i)>1]
    
    temp_dih2   = partial(update_D3, dihedrals_prot_fully_at_dih2)
    upd_D3_dih2 = pool.map(temp_dih2, iterable=list_survived_atoms)
    upd_D3_dih2 = [i for i in upd_D3_dih2 if len(i)>1]
    
    pool.close()
    pool.join()
    
    
    #D.4- Append in dihedrals_prot_fully_at_(dih2, dih4, dih9, tors) the type, at/cg, the NewType and substituting the NewIndex. 
    ...
    upd_D3b_dih4 = [i for i in upd_D3b_dih4 if len(i)>1]
    dihedrals_prot_fully_at_dih4 = upd_D3b_dih4
    
    
    temp_dih9    = partial(update_D3b, upd_D3_dih9)
    upd_D3b_dih9 = pool.map(temp_dih9, iterable=dihedrals_prot_fully_at_dih9)
    upd_D3b_dih9 = [i for i in upd_D3b_dih9 if len(i)>1]
    dihedrals_prot_fully_at_dih9 = upd_D3b_dih9
    
    
    temp_tors    = partial(update_D3b, upd_D3_tors)
    upd_D3b_tors = pool.map(temp_tors, iterable=dihedrals_prot_fully_at_tors)
    upd_D3b_tors = [i for i in upd_D3b_tors if len(i)>1]
    dihedrals_prot_fully_at_tors = upd_D3b_tors
    
    
    temp_dih2    = partial(update_D3b, upd_D3_dih2)
    upd_D3b_dih2 = pool.map(temp_dih2, iterable=dihedrals_prot_fully_at_dih2)
    upd_D3b_dih2 = [i for i in upd_D3b_dih2 if len(i)>1]
    dihedrals_prot_fully_at_dih2 = upd_D3b_dih2
    ...
    #D.5- After reading the dihedraltypes in ffbonded file,
    #     (dihedraltypes_dih4 = ffbon[2], dihedraltypes_dih9 = ffbon[3], dihedraltypes_tors = ffbon[4],  dihedraltypes_dih2 = ffbon[5])
    #     we append the value of the dihedral elastic constant (k_phi), the phase (phi0) and the multiplicity (n)
    #     for each quartet of survived atoms whose funtional form is 4 (dih4) [later we treat dih9, tors, and dih2]
    #
    ...
    dihedrals_prot_fully_at_dih4 = upd_D4_dih4
    
    pool.close()
    pool.join()
    
    
    #D.6- Sometimes (and it is not so rare) (it should not happens for dih4, but only for dih9 happens), some quartets of atoms are repeated more than once 
    ...
    for i in dihedrals_prot_fully_at_dih4:
        ...
        if(len(i) == 20): 
            d1 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[17],i[18],i[19], i[6],i[9],i[12],i[15]]
        ...
        if(len(i) == 23):
            d1 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[17],i[18],i[19], i[6],i[9],i[12],i[15]]
            d2 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[20],i[21],i[22], i[6],i[9],i[12],i[15]]
        ...
        if(len(i) == 26):
            d1 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[17],i[18],i[19], i[6],i[9],i[12],i[15]]
            d2 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[20],i[21],i[22], i[6],i[9],i[12],i[15]]
            d3 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[23],i[24],i[25], i[6],i[9],i[12],i[15]]
        ...
        if(len(i) == 29):
            ...
            d2 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[20],i[21],i[22], i[6],i[9],i[12],i[15]]
            d3 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[23],i[24],i[25], i[6],i[9],i[12],i[15]]
            d4 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[26],i[27],i[28], i[6],i[9],i[12],i[15]]
        ...
        if(len(i) == 32):
            ...
            d3 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[23],i[24],i[25], i[6],i[9],i[12],i[15]]
            d4 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[26],i[27],i[28], i[6],i[9],i[12],i[15]]
            d5 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[29],i[30],i[31], i[6],i[9],i[12],i[15]]
        ...
        if(len(i) == 35):
            ...
            d4 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[26],i[27],i[28], i[6],i[9],i[12],i[15]]
            d5 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[29],i[30],i[31], i[6],i[9],i[12],i[15]]
            d6 = [i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[32],i[33],i[34], i[6],i[9],i[12],i[15]]
    ...
    dihedrals_prot_fully_at_dih4 = [l for l in dihedrals_prot_fully_at_dih4 if len(l)==16]              # keep lists with lenght = 20,  after splitting
    ...
    dihedrals_prot_fully_at_dih9 = upd_D4_dih9
    ...
    dihedrals_prot_fully_at_dih9 = [l for l in dihedrals_prot_fully_at_dih9 if len(l)==16]              # keep lists with lenght = 20,  after splitting
    ...
    #D.8- After reading the special dihedraltypes defined as torsion (dihedraltypes_tors = ffbon[4]), 
    #     we append the value of the dihedral elastic constant (k_phi), the phase (phi0) and the multiplicity (n)
    #     for each quartet of "torsion" survived atoms.
    #
    #     Basically, "dihedraltypes_tors" and "dihedrals_prot_fully_at_tors" look like as follows: 
    ...
    dihedrals_prot_fully_at_tors = upd_D7
    
    pool.close()
    pool.join()
    
    dihedrals_prot_fully_at_tors =[[i[0],i[1],i[2],i[3],i[4], i[5], i[8],i[11],i[14],i[17], i[18],i[19],i[20], i[7],i[10],i[13],i[16]] \
    ...
    #D.9- About dih2, the situation is analogue to dih4 and dih9, but dih2 do not have multiplicity (n) that 
    #     is present in dih4 and dih9. Therefore we repeat only *D.4* section, keeping attention that j[7] of 
    #     "dihedraltypes_dih2" (multiplicity) is absent here. The *D.5* section is unnecessary because no quartet of atoms 
    #     can be repeated, thus no list-splitting according with the multiplicity must be performed. We have that:  
    #
    ...
    dihedrals_prot_fully_at_dih2 = upd_D8
    dihedrals_prot_fully_at_dih2 = [[i[0],i[1],i[2],i[3], i[4], i[7],i[10],i[13],i[16], i[17],i[18], i[6],i[9],i[12],i[15]] \
    ...
    #D.10- Creating the list dihedrals_prot_fully_at which is the sum of the three previous lists (dih4, dih9, tors & dih2) 
    ...
    #D.11- Creating a new list called "dihedrals_mult_res_prot" (in analogy with bonds, pairs, and angles) which is simply dihedrals_prot_fully_at, 
    
    dihedrals_mult_res_prot = dihedrals_prot_fully_at
    
    
    #D.12- When we need the input files for Lammps, dih4, dih9 and torsion require an harmonic series expansion of
    ...
    if(Flag_cmaps == True):   # i.e. the "original_cmaps_protein_fully_at" list is not-empty, i.e. the charmm.ff is employed  
        ...
        cmaps_prot_fully_at = [i for i in cmaps_prot_fully_at if i is not None]
        
    
        # E.2- Creating a tuple, containing for each atom involved in the cmap correction, the type, at/cg, NewIndex and the NewType 
        ...
        upd_E2 = [i for i in upd_E2 if len(i)>1]
        
        pool.close()
        pool.join()
    
        
        # E.3- Appending in 'cmaps_prot_fully_at' list the type, at/cg, NewType and substituting the NewIndex. 
        ...
        cmaps_prot_fully_at = upd_E2b
        
        pool.close()
        pool.join()
    
    
        #E.4- After reading the cmaptypes in "cmap.itp" file (cmaptypes = readcmap(f_cmap)), we append the func, grid1, grid2, and 
        #     the next 58 internal lists consisting of 576 values for the Torsional Correction Map for each quintet of survived atoms. 
        #
        #     Basically cmaptypes and cmaps_prot_fully_at look like as follow: 
        #
        ...
        cmaps_prot_fully_at = upd_E3
    
        pool.close()
        pool.join()
    
    
        # E.5- Rewrite 'cmaps_prot_fully_at' as for the CANVAS.py serial script. 
        for i in cmaps_prot_fully_at:
            del(i[6])
            del(i[8]) 
            del(i[10])
            ...
            i[6],i[7],i[8],i[9],i[10],i[11],i[12],i[13],i[14],i[15] = i[7],i[9],i[11],i[13],i[15], i[6],i[8],i[10],i[12],i[14]
        ...
        # E.6- Creating a new list called "cmaps_mult_res_prot" (in analogy with bonds, pairs, angles, and dihedrals) which is simply cmaps_prot_fully_at, 
        #      and sorting the list. 
    ...
    if(FlagLammps == True):
        ...
        if(solvate_Flag == True):            
            ...
            list_survived_atoms = upd_L5a_solv 
        ...
        if(solvate_Flag == False): 
            ...
            list_survived_atoms = upd_L5a_notsolv
        ...
        if(solvate_Flag == True):      
            ...
            list_survived_atoms = upd_L5c_1
     
    
    
            temp = partial(update_L5c_2, atoms_topol_ions)
            upd_L5c_2 = pool.map(temp, iterable=list_survived_atoms)
    
            list_survived_atoms = upd_L5c_2
        ...
        if(solvate_Flag == True): 
            ...
            for i in list_survived_atoms:
                if(i[8] == type_OW):
                    angles_SOL = [i[9] + 1, i[9], i[9] + 2]     			# bond fra OW e HW1 (indici j e j+1) 
