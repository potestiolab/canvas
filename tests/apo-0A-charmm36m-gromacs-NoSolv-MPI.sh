...
echo $CHARMM_PATH | python3 $PYTHONDIR/CANVAS-MPI4.py -g $inputDIR/apo-0A.gro -l list-atoms-opt-2.txt -t $inputDIR/topol.top -d $inputDIR/dom.txt -c lammps -s n

rm -rf test-apo-0A-charmm36m-lammps-NoSolv
mkdir test-apo-0A-charmm36m-lammps-NoSolv

mv run_simulation/ other-files/ analysis/ test-apo-0A-charmm36m-lammps-NoSolv
