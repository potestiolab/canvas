...
CHARMM_PATH=../input-files/PEMBROLIZUMAB-ANTIBODY/sim-4A-charmm36m/charmm36-jul2021.ff

PYTHONDIR=../PYTHON-scripts

inputDIR=../input-files/PEMBROLIZUMAB-ANTIBODY/sim-4A-charmm36m

python3 $PYTHONDIR/block-MPI.py choice2 -g $inputDIR/apo-4A.gro -l $inputDIR/list-ATres-2nd-choice.txt 

echo $CHARMM_PATH | python3 $PYTHONDIR/CANVAS-MPI4.py -g $inputDIR/apo-4A.gro -l list-atoms-opt-2.txt -t $inputDIR/topol.top -d $inputDIR/dom.txt -s n 

rm -rf test-apo-4A-charmm36m-gromacs-NoSolv 
mkdir test-apo-4A-charmm36m-gromacs-NoSolv

mv run_simulation/ other-files/ analysis/ test-apo-4A-charmm36m-gromacs-NoSolv 
