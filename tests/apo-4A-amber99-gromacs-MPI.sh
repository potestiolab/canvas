...
inputDIR=../input-files/PEMBROLIZUMAB-ANTIBODY/sim-4A-amber99

python3 $PYTHONDIR/block-MPI.py choice2 -g $inputDIR/apo-4A.gro -l $inputDIR/list-ATres-2nd-choice.txt 

python3 $PYTHONDIR/CANVAS-MPI4.py -g $inputDIR/apo-4A.gro -l list-atoms-opt-2.txt -t $inputDIR/topol.top -d $inputDIR/dom.txt 

rm -rf test-apo-4A-amber99-gromacs
mkdir test-apo-4A-amber99-gromacs

mv run_simulation/ other-files/ analysis/ test-apo-4A-amber99-gromacs 
