...
def write_lmp_file(f_lmp, number_of_atoms, number_of_bonds, number_of_angles, number_of_dihedrals, number_of_impropers, number_atom_types, \
    ...
    upd_mass = [i for i in upd_mass if i is not None]

    upd_mass = list(set([i for i in upd_mass])) # remove duplicates in list of tuples
    upd_mass.sort(key=itemgetter(0))            # ...and sort the 1st element 
    ...
    upd_b_coeffs = [i for i in upd_b_coeffs if i is not None]

    upd_b_coeffs = list(set([i for i in upd_b_coeffs]))  # remove duplicates in list of tuples
    upd_b_coeffs.sort(key=itemgetter(0))                 # ...and sort the 1st element 

    for i in upd_b_coeffs:
    ...
    upd_a_coeffs = [i for i in upd_a_coeffs if i is not None]

    upd_a_coeffs = list(set([i for i in upd_a_coeffs]))           # remove duplicates in list of tuples
    upd_a_coeffs.sort(key=itemgetter(0))                          # ...and sort the 1st element 

    for i in upd_a_coeffs:
        if(len(i)==3):
            f_lmp.write("{} harmonic {:17.8e} {:17.8e}\n".format(i[0],i[1],i[2]))
    ...
    upd_d_coeffs = [i for i in upd_d_coeffs if i is not None]

    upd_d_coeffs = list(set([i for i in upd_d_coeffs]))           # remove duplicates in list of tuples
    upd_d_coeffs.sort(key=itemgetter(0))                          # ...and sort the 1st element 


    for i in upd_d_coeffs:
        if(len(i)==6):
    ...
    upd_i_coeffs = [i for i in upd_i_coeffs if i is not None]

    upd_i_coeffs = list(set([i for i in upd_i_coeffs]))           # remove duplicates in list of tuples
    upd_i_coeffs.sort(key=itemgetter(0))                          # ...and sort the 1st element 

    for i in upd_i_coeffs:
        f_lmp.write("{} harmonic {:17.8e} {:17.8e}\n".format(i[0],i[1],i[2]))
...
def write_input_lammps_file(f_input, dihedrals_mult_res_prot, Flag_cmaps, number_of_bonds, number_of_angles, number_of_dihedrals, \
    ...
    upd_L16b = sorted(upd_L16b, key=itemgetter(0,1))                                              # sorting 1st, and then 2nd column   
    upd_L16b =list(upd_L16b for upd_L16b,_ in itertools.groupby(upd_L16b))                        # remove duplicates if present
    ...
    for i in list_survived_atoms:
        if(i[7] == "at" or i[7] =="AT" or i[7] =="At"):   # Writing only atomTypes and n_atomType for AT atoms as CG beads 
							  # does not have sigma14 and eps14 for charmm.ff and will be deleted later. 

            list_number_type.append([i[8], i[-1]])   #i[8] = atomType ; i[-1] = number_of_corresponding_atomType 


    list_number_type = sorted(list_number_type, key=itemgetter(1))                                              # sorting 2nd column (n-atomType)
    ...
    ##k- Writinf last parameters: groups, fix parameters, thermo_style, timestep, and so on...
    if(solvate_Flag == True): 
